...
# Plotting
# ---------------------------------------------------------------------
def plot_inbound_outbound(df: pd.DataFrame) -> None:
    """Plot inbound vs outbound altitudes and latitudes."""
    plt.figure(figsize=(10, 6))
    plt.scatter(df["INBOUND_LATITUDE"], df["INBOUND_ALTITUDE"], s=20, c="blue", alpha=0.6, label="Inbound")
    plt.scatter(df["OUTBOUND_LATITUDE"], df["OUTBOUND_ALTITUDE"], s=20, c="red", alpha=0.6, label="Outbound")
...
if __name__ == "__main__":
    ...
    print(df.head(10)[["ORBIT", "DATETIME_UTC", "INBOUND_LATITUDE", "INBOUND_ALTITUDE",
                       "OUTBOUND_LATITUDE", "OUTBOUND_ALTITUDE"]])

    plot_inbound_outbound(df)
